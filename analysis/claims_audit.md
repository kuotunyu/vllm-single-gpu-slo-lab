...
|---|---|---|---|---|---|---|
| 1 | r_SLO（TTFT p95 ≤ 1 s ∧ TPOT p95 ≤ 50 ms）：BF16 10.26、FP8 26.2、AWQ 22.61、GPTQ-Int4 22.70 req/s | `evidence/raw/w2/<cell>/open-loop/seed-{1,2,3}/ol-rate-*/records.jsonl`；`analysis/tables/w2-<cell>-open-loop/open_loop.json` `per_cell.<cell>.r_slo` | FP8 3 seeds × 11 rates；其他 3 seeds × 14 rates（含 0.80–0.90 × r_sat 補點）；r_SLO 點每 seed 3,078–7,860 筆 | 每點 attainment Wilson 95%（表內）；r_SLO 本身無 CI（凍結規則取所有 seed 的 min），以「下一格」夾擊：+7–11% | 單卡、單一 prompt 形狀；不外推 | 2026-09-11 `reproduce-lite` 重建一致 |
