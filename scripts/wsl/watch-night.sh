...
stdbuf -o0 tail -F -n +1 "$LOG" 2>/dev/null | stdbuf -o0 grep -E \
  "server ready|CELL DONE|NIGHT DONE|START |r_sat=|r_slo |no r_sat|TMMLU done|promoted -> |quarantined|SERVER_NOT_READY|SERVER_EXITED_EARLY|QUIET_GPU_REFUSED|Traceback|Error|error:|Killed|OOM|exit=[1-9]"
