...
.venv/bin/vllm serve "$MODEL" --host 127.0.0.1 --port 8013 --max-model-len 4096 --gpu-memory-utilization "$GPU_MEM_UTIL" --max-num-seqs "$MAX_NUM_SEQS" --max-num-batched-tokens 2048 $EXTRA > "$BATCH/serve.log" 2>&1 &
PID=$!
...
FLAGS_JSON=$(printf '{"model":"%s","max_model_len":4096,"gpu_memory_utilization":%s,"max_num_seqs":%s,"max_num_batched_tokens":2048,"extra":"%s"}' "$MODEL" "$GPU_MEM_UTIL" "$MAX_NUM_SEQS" "$EXTRA")
first=1
for spec in "$@"; do
