...
for i in $(seq 1 "${IO_MAX_SAMPLES:-480}"); do
  ...
  [ $i -gt 6 ] && { pgrep -f "${IO_DRIVER_PATTERN:-wsl/batch.sh|wsl-batch.sh}" >/dev/null || { echo "driver gone at sample $i"; exit 0; }; }
  sleep 10
done
echo "sampler timeout (${IO_MAX_SAMPLES:-480} samples)"
