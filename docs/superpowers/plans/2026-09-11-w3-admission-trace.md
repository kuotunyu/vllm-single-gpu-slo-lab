...
| 04:31 | Smoke 2 gate SMOKE_OK (passthrough vs direct pre-burst TPOT p95 37.2 vs 39.2 ms, TTFT p95 105 vs 95 ms; bounded 1,851 queue timeouts recorded as 429). FP8 main cell started; KV 76,112 tokens in all three sessions. |
| 04:31-09:01 | FP8, 9 stages, all clean (probe TPOT 19.8-20.0 ms, committed <= 22.3 GB, zero timeouts, records = trace rows - 1). No suspects, promoted to `evidence/raw/w3/fp8/trace/`. |
| 09:01-13:04 | BF16, 9 stages, all clean (KV 29,696 tokens; probe 18.6-19.6 ms, seed 3 session about 5 % slower while the desktop was in use). No suspects, promoted. `W3 NIGHT DONE` 13:04:42; Windows VRAM max 22,604 MB over 1,040 samples. |
| 13:10 | Wrap-up: tables registered in `analysis/tables/index.json`, reproduce deterministic, ADR 0013 results, README, claims audit rows 14-16, evidence README. User asked for the full W4 later, on a GPU window they will schedule. |
